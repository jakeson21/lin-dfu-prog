...
Bin2DfuWrapper::ReadInputFile(const std::string& pcFilename, bool bHdrs, uint32_t *pulLength)
{
	...
	if((fhFile = fopen(pcFilename.c_str(), "rb")) == 0)
    {
        //
...
Bin2DfuWrapper::IsSuffixValid(unsigned char *pcData, unsigned char *pcEnd)
{
    ...
    {
        VERBOSEPRINT("Read CRC 0x%08ulx, calculated 0x%08ulx.\n", ulCRCRead, ulCRCCalc);
...
Bin2DfuWrapper::DumpPrefix(unsigned char *pcPrefix)
{
    ...
    QUIETPRINT("Payload length: %uld (0x%ulx) bytes, %uldKB\n", ulLength,
...
Bin2DfuWrapper::WriteOutputFile(const std::string& pszFile, unsigned char *pcData, uint32_t ulLength)
{
    ...
    if(!g_bOverwrite)
    {
        ...
        //
        if((fh = fopen(pszFile.c_str(), "rb")) == 0)
        {
    ...
    //
	if((fh = fopen(pszFile.c_str(), "wb")) == 0)
    {
    ...
    {
        QUIETPRINT("Error writing data to output file! Wrote %uld, "
                   "requested %uld\n", ulWritten, ulLength);
        return(9);
    }
    else
    {
        QUIETPRINT("Output file written successfully (%d bytes) ===> \"%s\"\n", ulWritten, pszFile.c_str());
    }
