		...
		if (has_dfu) {
			...
			if (libusb_open(dev, &devh) == 0) {
				ret = libusb_get_descriptor(devh, USB_DT_DFU, 0,
						reinterpret_cast<unsigned char *>(&func_dfu), sizeof(func_dfu));
				libusb_close(devh);
				if (ret > -1)
					goto found_dfu;
			}
			...
			for (alt_idx = 0;
				...
				if (intf->iInterface != 0)
					ret = get_string_descriptor_ascii(devh,
					    intf->iInterface, reinterpret_cast<unsigned char *>(&alt_name), MAX_DESC_STR_LEN);
				else
					ret = -1;
				if (ret < 1)
				...
				if (desc->iSerialNumber != 0)
					ret = get_string_descriptor_ascii(devh,
					    desc->iSerialNumber, reinterpret_cast<unsigned char *>(&serial_name), MAX_DESC_STR_LEN);
				else
					ret = -1;
				if (ret < 1)
				...
				pdfu = static_cast<dfu_if*>(dfu_malloc(sizeof(*pdfu)));
